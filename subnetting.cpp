...
vector<int> decimal(vector<int> bi)
{
    vector<int> arr(4,0);
    int a, b, c, d, i, j;
    a = b = c = d = 0;
...
int main()
  {
    ...
    // You can take user input here instead of using default address
    ...
    // Obtaining Broadcast address by setting 32-n bits to 1
    for (int i = 31; i > (31 - t); i--) {
      brd[i] = 1;
    }
    ...
    vector<int> nt = decimal(ntwk);

    // Converting broadcast address to decimal
    vector<int> br = decimal(brd);
